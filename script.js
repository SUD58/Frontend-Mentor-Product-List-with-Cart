...
function addToCart(button, index) {
  ...
  cartItemIndex = cartItems.indexOf(items[index]);
  cartItems[cartItemIndex].quantity = 1; //Set item quantity
  cartItems[cartItemIndex].id = index;
  console.log(cartItems[cartItemIndex].id);
...
function increment(buttonDiv) {
  const incrementButton = buttonDiv.querySelector(".increment-button");
  const itemQuantityParagraph = buttonDiv.querySelector("p");
  incrementButton.addEventListener("click", () => {
    const cartItemIndex = cartItems.findIndex(
      (cartItem) => cartItem.id == buttonDiv.dataset.itemId,
    );

    cartItems[cartItemIndex].quantity++;
    cartQuantity++;
    cartQuantitySpan.textContent = cartQuantity;
    itemQuantityParagraph.textContent = cartItems[cartItemIndex].quantity;
    populateCartItemsList(cartItems, cartItemsList, buttonDiv);
    setTotalPrice(cartItems);
  });
}

function decrement(buttonDiv) {
  const decrementButton = buttonDiv.querySelector(".decrement-button");
  const itemQuantityParagraph = buttonDiv.querySelector("p");
  const addedToCartDiv = buttonDiv.querySelector(".added-to-cart-div");
